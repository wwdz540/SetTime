...
public class ConfigCache {
	...
	public static String getUrlCache(String url) {
		...
		if (file.exists() && file.isFile()) {
			...
			if (BaseApplication.mNetWorkState != NetUtil.NETWORN_NONE
					&& expiredTime < 0) {
				return null;
			}
			//如果是wifi网络，则30分钟过期
			if (BaseApplication.mNetWorkState == NetUtil.NETWORN_WIFI
					&& expiredTime > CONFIG_CACHE_WIFI_TIMEOUT) {
				return null;
				//如果是手机网络，则2个小时过期
			} else if (BaseApplication.mNetWorkState == NetUtil.NETWORN_MOBILE
	...
	public static void setUrlCache(String data, String url) {
		if (BaseApplication.getInstance().getCacheDir() == null) {
			return;
		}
